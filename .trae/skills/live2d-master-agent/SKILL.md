...
| 优先级 | 方案 | 条件 | 质量 | 成本 |
|--------|------|------|------|------|
| 1 | ComfyUI 本地 | 已安装且运行中 | ⭐⭐⭐⭐⭐ | 免费 |
| 2 | Seedream API | API 已配置 | ⭐⭐⭐⭐ | 按量计费 |
| 3 | Playground AI | 在线免费 | ⭐⭐⭐⭐ | 免费（有额度） |
| 4 | 手动上传 | 始终可用 | 用户提供 | 免费 |
...
用户请求生成角色立绘
     ↓
检测 ComfyUI 是否运行中
     ↓ (是) → 使用 ComfyUI 生成（最高质量）
     ↓ (否)
检测 ComfyUI 是否已安装
     ↓ (是) → 提示启动或自动启动
     ↓ (否)
检测 Seedream API 是否已配置
     ↓ (是) → 使用 Seedream API
     ↓ (否)
引导使用 Playground AI（在线免费）
     ↓
手动上传图片作为备用
```
