  ...
  return (
    <div
      className={`
        ...
        ${dragOver
          ? 'bg-gradient-to-br from-pink-500/20 via-purple-500/20 to-blue-500/20 border-2 border-pink-400/50 scale-[1.02]'
    ...
    >
      ...
      <div className="absolute -bottom-24 -left-24 w-48 h-48 bg-gradient-to-br from-blue-500/20 to-purple-500/20 rounded-full blur-3xl pointer-events-none" />
      ...
      {loading ? (
        <div className="py-16 px-8 flex flex-col items-center justify-center animate-pulse">
          <div className="text-pink-400 mb-4">
            <SpinnerIcon />
          </div>
          <p className="text-gray-300 font-medium text-lg mb-2">正在分析 PSD 文件</p>
          <p className="text-gray-500 text-sm">请稍候...</p>
          
          <div className="mt-8 w-full max-w-xs h-1 bg-gray-700/50 rounded-full overflow-hidden">
        ...
        <div className="py-12 px-8 flex flex-col items-center justify-center">
          <div className="text-green-400 mb-4 transform scale-110">
            <CheckIcon />
          </div>
          <p className="text-white font-semibold text-lg mb-2">{fileInfo.name}</p>
          <div className="flex items-center gap-4 text-sm text-gray-400 mb-4">
            <span className="px-3 py-1 bg-gray-700/50 rounded-lg">{formatSize(fileInfo.size)}</span>
            <span className="px-3 py-1 bg-gray-700/50 rounded-lg">{fileInfo.width} × {fileInfo.height}</span>
          </div>
        ...
        <div className="py-16 px-8 flex flex-col items-center justify-center">
          <div className={`mb-6 transition-all duration-500 ${dragOver ? 'text-pink-400 scale-110' : 'text-gray-500'}`}>
            <UploadIcon />
          </div>
          ...
          <div className="flex items-center gap-2 text-xs text-gray-500">
            <span className="px-3 py-1.5 bg-gray-700/50 rounded-lg border border-gray-600/30 flex items-center gap-2">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              Photoshop PSD
            </span>
            <span className="px-3 py-1.5 bg-gray-700/50 rounded-lg border border-gray-600/30">
              最大 50MB
            </span>
          </div>
