...
本项目采用 **[MIT 许可证](LICENSE)**（v9.0 原版许可，已于 2026-07-30 恢复），这是最宽松的开源协议之一，适合代码、算法、架构等工程类资产。

### MIT 许可说明（通俗版）💡

**✅ 您可以自由地：**
- **商业使用** — 用在闭源商业产品、付费 SaaS、企业内部生产工具、付费定制服务里（无需告知原作者）
- **修改** — 对源代码、算法、前端、后端任意修改、二次开发、删改模块
- **分发** — 公开或私下分发、打包成安装包、上架应用商店、集成进更大的软件栈
- **私人使用** — 个人学习、研究、私下使用完全无限制
- **再许可** — 衍生作品可以换任何协议（可继续 MIT，也可以切到 GPL、Apache、BSD 或闭源）

**⚠️ 只有一条义务（必须遵守）：**
- **保留版权与许可声明** — 在你再分发的每一份源代码副本/二进制打包产物里，**必须保留原作者的版权声明和本 MIT 许可全文**（即根目录 `LICENSE` 文件的内容）。仅口头提及或在网页里提一句"参考了 Live2D Master Agent"是不够的，必须把 License 文本/URI 放进每一份副本里。

**🚫 没有任何其他限制：**
- 不要求署名（但保留版权声明即可，属于"软署名"）
- 不要求注明修改内容（MIT 没这个要求，但是良好的工程实践可以写在 CHANGELOG 里）
- 不禁止商用、不禁止二次售卖、不禁止企业内部生产使用
- 衍生产品不需要开原（可以完全闭源）

### 产权声明与免责 ©️
- 本项目所有原始内容的著作权与知识产权归 **Live2D Master Agent Team** 所有；MIT 协议是版权所有者向被许可方发放的"使用授权"，**版权不会因此转让**
- **本软件按"原样"提供**：版权所有者不提供任何明示或暗示的担保，包括但不限于适销性、特定用途适用性和非侵权性。任何情况下，版权所有者或贡献者对因使用本软件或与本软件相关的任何索赔、损害或其他责任概不负责（MIT 协议标准免责条款）
- 如需额外的商业支持、专属定制服务、SLA 保障或书面授权文件，欢迎联系产权所有者

> 🔄 **版本沿革提示**：本项目在 2026-07-30 曾短暂切换至 CC BY-NC 4.0（Rev.1）→ CC BY-NC 2.0（Rev.2）以验证严格许可模式；经产权所有者评估，最终决定恢复 **v9.0 原版 MIT 协议**（Rev.3 / 现行），以最大化社区贡献活力与商用场景兼容性。按 CC BY-NC 2.0 发布的历史版本（commit `65fb3b4` 之前）对接收者永久有效，不追溯。

### 完整法律文本
- MIT 协议全文：见项目根目录 `LICENSE` 文件
- OSI 官方说明：https://opensource.org/license/MIT
- SPDX 标识符：`MIT`
